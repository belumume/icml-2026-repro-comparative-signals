...
property of that regime and not of the harness. And the specific suspicion above, that the
bootstrap interval understates rerun-to-rerun uncertainty, is **refuted**: the mean
within-run interval is *wider* than the across-run spread implies, by a factor of about 2
at σ = 0.08 and 11 at σ = 1.0. Those intervals are conservative, not optimistic.
...
**One caveat does survive, and it is about magnitude rather than direction.** The R = 100
replicates centre on −0.18, and the −0.3300 this page led with sits outside their whole
range. The sign of the low-noise result is solid, and its size depends on R in a way this
reproduction has not characterised, so it should be read as "negative, order tenths" rather
than as a constant. The ablation table above is unaffected either way, because it is a
...
- **Every number above is checked against the published results in about a second, with no setup.**
  ...
  published [`results/`]({RESULTS_BASE}) JSON and checks all 36 headline figures
